        ...
        if isinstance(section_content, dict):
            ...
            for subsection, subsection_content in section_content.items():
                if subsection != 'content':  # Skip the 'content' key used for metadata
                    ...
                    # Create a table for subsection content
                    data_table = [[Paragraph(str(subsection_content), normal_style)]]
                    subsection_table = Table(data_table, colWidths=[doc.width - 40])
                    ...
                    content.append(subsection_table)
                    content.append(Spacer(1, 10))
        else:
            # Create a table for the main section content
            formatted_content = str(section_content).replace('•', '⚫')  # Replace bullet points with circles
            data_table = [[Paragraph(formatted_content, normal_style)]]
...
def process_input(user, is_append=False):
    ...
    while True:
        ...
        if choice == "1":
            ...
            if current_user:
                ...
                process_input(current_user, is_append)
            else:
