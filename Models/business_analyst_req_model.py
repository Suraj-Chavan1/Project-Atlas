...
async def process_ba_template(project_id, requirements_text):
    """Process and generate Business Analyst template content."""
    try:
        # Generate content
        ...
        requirements_data = {
            "id": doc_id,
            "template_type": "business_analyst",
        ...
        print(f"Error processing Business Analyst template: {str(e)}")
        raise
...
async def main():
    """Main function to process Business Analyst template using document input."""
    try:
        ...
        # Generate Business Analyst requirements
        print(f"\nGenerating Business Analyst requirements for project: {project_name}")
        doc_id, requirements_data = await process_ba_template(project_id, requirements_text)
        
        # Generate PDF
        print("\nGenerating PDF document...")
        pdf_path = f"BA_Requirements_{project_id}.pdf"
        generate_pdf(requirements_data["content"], pdf_path)
        
        # Upload to Blob Storage
        print("\nUploading PDF to Blob Storage...")
        blob_name = f"ba_requirements/{project_id}/{pdf_path}"
