...
async def generate_sde_content(requirements_text, openai_client, deployment_name):
    """Generate content for SDE template sections using Azure OpenAI."""
    content = {}
    for section in SDE_SECTIONS:
        prompt = f"""
        You are a Software Development Engineer working on a project. 
        ...
        try:
            response = openai_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are a Software Development Engineer creating brief technical specifications."},
...
def generate_pdf(data, output_buffer):
    ...
    try:
        ...
        content.append(Paragraph("Software Development Requirements", title_style))
        ...
        def add_page_number(canvas, doc):
            ...
            canvas.drawString(doc.leftMargin, 25, "Software Development Requirements")
            canvas.restoreState()
        ...
        try:
            ...
            c.drawString(50, y, "Software Development Requirements")
            y -= 30
...
async def process_sde_template(project_id, requirements_text):
    """Process and generate SDE template content."""
    try:
        # Check for existing document
        container = await get_cosmos_container()
        query = f"SELECT * FROM {CONTAINER_NAME} c WHERE c.project_id = '{project_id}' AND c.template_type = 'sde'"
        items = [item async for item in container.query_items(query=query)]
        
        # Generate content
        content = await generate_sde_content(
            requirements_text,
            openai_client,
            DEPLOYMENT_NAME
        )
        ...
        if items:
            ...
            # Create new document
            doc_id = f"sde_requirements_{project_id}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
            requirements_data = {
                "id": doc_id,
                "template_type": "sde",
                "content": content,
        ...
        print(f"Error processing SDE template: {str(e)}")
        print(f"Error type: {type(e).__name__}")
        print(f"Traceback: {traceback.format_exc()}")
        raise

def generate_simple_pdf(content: str, output_path: str) -> None:
    """
...
# Create Blueprint
sde = Blueprint('sde', __name__, url_prefix='/sde')
CORS(sde)  # Enable CORS for all routes in this blueprint

@sde.route('/generate-document', methods=['POST'])
async def generate_sde_document():
    try:
        print("\n=== Starting SDE document generation ===")
        ...
        print(f"Processing SDE template for project {project_id}")
        ...
        try:
            ...
            result = await process_sde_template(project_id, requirements_text)
        ...
        print("\nError in generate_sde_document:")
...
@sde.route('/get-document/<project_id>', methods=['GET'])
async def get_sde_document(project_id):
    print(f"Fetching SDE document for project {project_id}")
    try:
        # Get the event loop
        ...
        query = f"SELECT * FROM c WHERE c.project_id = '{project_id}' AND c.template_type = 'sde'"
        items = []
        ...
        if items:
            ...
            return jsonify({
                'success': True,
                'exists': False,
                'message': 'No SDE document found for this project'
            })

    except Exception as e:
        print(f"Error fetching SDE document: {str(e)}")
...
@sde.route('/edit-with-ai/<project_id>', methods=['POST'])
async def edit_with_ai(project_id):
    try:
        ...
        prompt = f"""
        You are a technical documentation expert. Please help edit the following text based on the provided context.
...
@sde.route('/update-document/<project_id>', methods=['PUT', 'OPTIONS'])
async def update_sde_document(project_id):
