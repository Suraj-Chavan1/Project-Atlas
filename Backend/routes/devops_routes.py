...
async def process_devops_template(project_id, requirements_text):
    ...
    try:
        ...
        query = f"SELECT * FROM {CONTAINER_NAME} c WHERE c.project_id = '{project_id}' AND c.template_type = 'devops'"
...
async def generate_devops_document():
    try:
        print("\n=== Starting DevOps document generation ===")
        ...
        print(f"Processing DevOps template for project {project_id}")
        ...
        try:
            ...
            result = await process_devops_template(project_id, requirements_text)
        ...
        print("\nError in generate_devops_document:")
...
@devops.route('/get-document/<project_id>', methods=['GET'])
async def get_devops_document(project_id):
    print(f"Fetching DevOps document for project {project_id}")
    try:
        # Get the event loop
        ...
        if items:
            ...
            return jsonify({
                'success': True,
                'exists': False,
                'message': 'No DevOps document found for this project'
            })

    except Exception as e:
        print(f"Error fetching DevOps document: {str(e)}")
