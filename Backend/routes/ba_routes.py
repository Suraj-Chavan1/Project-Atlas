...
async def generate_ba_content(requirements_text, openai_client, deployment_name):
    """Generate content for Business Analyst template sections using Azure OpenAI."""
    content = {}
    for section in BA_SECTIONS:
        prompt = f"""
        You are a Business Analyst working on a project. 
        ...
        Focus only on the most important business details.
        ...
        try:
            response = openai_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are a Business Analyst creating brief business specifications."},
...
def generate_pdf(data, output_buffer):
    ...
    try:
        ...
        content.append(Paragraph("Business Analysis Requirements", title_style))
        ...
        def add_page_number(canvas, doc):
            ...
            canvas.drawString(doc.leftMargin, 25, "Business Analysis Requirements")
            canvas.restoreState()
        ...
        try:
            ...
            c.drawString(50, y, "Business Analysis Requirements")
            y -= 30
...
async def process_ba_template(project_id, requirements_text):
    """Process and generate BA template content."""
    try:
        # Check for existing document
        container = await get_cosmos_container()
        query = f"SELECT * FROM {CONTAINER_NAME} c WHERE c.project_id = '{project_id}' AND c.template_type = 'ba'"
        items = [item async for item in container.query_items(query=query)]
        
        # Generate content
        content = await generate_ba_content(
            requirements_text,
            openai_client,
            DEPLOYMENT_NAME
        )
        ...
        blob_url = upload_to_blob_storage_from_memory(pdf_buffer, 'ba', str(uuid.uuid4()))
        ...
        if items:
            ...
            # Create new document
            doc_id = f"ba_requirements_{project_id}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
            requirements_data = {
                "id": doc_id,
                "template_type": "ba",
                "content": content,
        ...
        print(f"Error processing BA template: {str(e)}")
        print(f"Error type: {type(e).__name__}")
        print(f"Traceback: {traceback.format_exc()}")
        raise
...
# Create Blueprint
ba = Blueprint('ba', __name__, url_prefix='/ba')
CORS(ba)  # Enable CORS for all routes in this blueprint

@ba.route('/generate-document', methods=['POST'])
async def generate_ba_document():
    try:
        print("\n=== Starting BA document generation ===")
        ...
        print(f"Processing BA template for project {project_id}")
        ...
        try:
            ...
            result = await process_ba_template(project_id, requirements_text)
...
@ba.route('/get-document/<project_id>', methods=['GET'])
async def get_ba_document(project_id):
    print(f"Fetching BA document for project {project_id}")
    try:
        # Get the event loop
        ...
        query = f"SELECT * FROM c WHERE c.project_id = '{project_id}' AND c.template_type = 'ba'"
        items = []
        ...
        if items:
            ...
            return jsonify({
                'success': True,
                'exists': False,
                'message': 'No BA document found for this project'
            })

    except Exception as e:
        print(f"Error fetching BA document: {str(e)}")
...
@ba.route('/edit-with-ai/<project_id>', methods=['POST'])
async def edit_with_ai(project_id):
    try:
        ...
        prompt = f"""
        You are a business analyst documentation expert. Please help edit the following text based on the provided context.
        ...
        try:
            ...
            response = openai_client.chat.completions.create(
                model=DEPLOYMENT_NAME,
                messages=[
                    {"role": "system", "content": "You are a business analyst documentation expert helping to improve business documents."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.7,
                max_tokens=2000
...
@ba.route('/update-document/<project_id>', methods=['PUT', 'OPTIONS'])
async def update_ba_document(project_id):
    ...
    try:
        print("\n=== Starting BA document update process ===")
        print("Request headers:", dict(request.headers))
        print("Request data:", request.json)
        print(f"Project ID: {project_id}")
        ...
        try:
            ...
            doc_content.append(Paragraph("Business Requirements Document", title_style))
            ...
            def add_page_number(canvas, doc):
                ...
                # Add document title in footer
                canvas.drawString(doc.leftMargin, 25, "Business Requirements Document")
                canvas.restoreState()
